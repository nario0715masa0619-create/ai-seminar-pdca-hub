...
| cta_text | CTAボタン等の文言 |
| status | proposed / approved / rejected / paused_auto / winner のいずれか（Sheets側でプルダウン検証を設定済み、`scripts/setup-status-dropdowns.js`参照） |
...
| status | proposed / approved / rejected のいずれか（Sheets側でプルダウン検証を設定済み、`scripts/setup-status-dropdowns.js`参照） |

## weekly_pdca_log Sheet (Weekly PDCA Automation)

**注意**: セミナー単位のシート（Parent/X_ads_ops/LPs等）とは別系統。X広告の**週次**PDCA自動化
（`docs/weekly-post-pdca-ops.md`、`docs/weekly-pdca-n8n-workflow.json`参照）専用のログシート
（テンプレキー・Google Sheets上のタブ名ともに `weekly_pdca_log`）。
1行＝1週間分の実行ログとする。n8nからのappend専用（人間は基本的に追加・編集しない）。

### Required Columns

| column | description |
|---|---|
| week | 対象週を表すラベル（例: 2026-07-08〜2026-07-14） |
| run_at | PDCAワークフロー実行日時（ISO形式） |
| ctr | 今週のCTR（url_clicks / impressions） |
| signups | 今週のsignups（セミナー申込数） |
| prev_ctr | 前週のCTR（初週は空） |
| prev_signups | 前週のsignups（初週は空） |
| status | PDCA判定結果（baseline / keep / replace_post / improve_lp / replace_post_and_improve_lp） |
| reason | 上記statusになった理由（人間が読める文章） |
| next_post_A | 次週に使う投稿案A（X広告の文面） |
| next_post_B | 次週に使う投稿案B（X広告の文面） |
| lp_suggestions | LP改善案（複数ある場合は"/"区切りで1セルに格納） |

## pdca_state Sheet (Weekly PDCA Automation)

週次PDCAの「前回状態」を1行だけ保存するstate専用シート（テンプレキー・タブ名ともに `pdca_state`）。
n8nからのread/appendOrUpdate専用（人間は通常編集しない）。初期セットアップ時に、`row_key=current`の
1行を手動で作成しておく必要がある（`docs/weekly-post-pdca-ops.md`参照）。

### Required Columns

| column | description |
|---|---|
| row_key | 常に"current"固定。n8nのappendOrUpdateで行を一意に特定するためのキー |
| prev_ctr | 前回実行時のCTR |
| prev_signups | 前回実行時のsignups |
| last_status | 直近のPDCA status（参考情報） |
| updated_at | この行が最後に更新された日時 |
